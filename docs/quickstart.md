...
Ingest a bounded course sample with `.calvin-autonomy/bin/coursework --workspace workspace ingest PATH --course COURSE_ID --max-files 50 --max-pages 100`, then search with `.calvin-autonomy/bin/coursework --workspace workspace search "topic or method" --course COURSE_ID`. Search results include the document ID, source hash, locator, extracted-text hash, and durable review status. Image files and image-only PDF pages stay visibly unresolved with hash-bound OCR review manifests; `ocr-status --course COURSE_ID` lists them and reports whether a local OCR engine is present without running it.
