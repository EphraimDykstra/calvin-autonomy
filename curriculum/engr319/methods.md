...
- **Homework:** the weekly prompts survive, carrying day-by-day topic structure, full problem statements, instructional asides, and a supplied answer for most problems. Their own text states the presentation rules: the course is focused on submitting **hand solutions**; equation-solver work is for the problems a set requests it for, printed with the code, plots and solution window and with the textbook value beside the solver's where they are compared; the table behind each table-practice answer is named; and a control-volume sketch goes on every problem. The student's own worked pages also survive, in digital ink on or after the prompt pages, and were read as page images. They show a layout (givens with units, the governing relation before any number, inlet state on the left and outlet on the right, the final value boxed) that is recorded as **observed practice, not a requirement**. None carries a grader mark, a score or feedback, so what earned credit is still unknown.
